 ...
 * @Description:
 * @Author: YanCheng
 * @Date: 2021-12-13 11:39:33
 * @LastEditTime: 2021-12-14 16:40:10
 */
...
const now = new Date("2030/01/01 00:00:00");
const years = [];
...
Component({
  ...
  methods: {
    ...
    handleCancel() {
      ...
      this.valueArray = this.data.valueArray;
    ...
    handleValueChange() {
      ...
      for (let i in valueArray) {
        let v = this.data.rangeValues[i][valueArray[i]];
    ...
    onTimeChange(e) {
      const { value: tempArr } = e.detail;
      const { valueArray } = this;
      this.valueArray = tempArr;
      // 不需要处理月份
      if (valueArray[0] === tempArr[0] && valueArray[1] === tempArr[1])
        return false;
      let year = this.data.pickerYear;
      let month = this.data.pickerMonth;
      if (valueArray[0] !== tempArr[0]) {
        // 年变化
        year = tempArr[0] + beginYear;
      } else if (valueArray[1] !== tempArr[1]) {
        // 月变化
        month = tempArr[1] + 1;
      }
